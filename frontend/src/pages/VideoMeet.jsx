...
export default function VideoMeetComponent() {
    ...
    // ----- Lobby device pre-toggles -----
    let toggleLobbyVideo = () => {
        const next = !videoAvailable;
        setVideoAvailable(next);
        try {
            localVideoref.current?.srcObject?.getVideoTracks().forEach(t => (t.enabled = next));
        } catch (e) { }
    }
    let toggleLobbyAudio = () => {
        const next = !audioAvailable;
    ...
    const meetingCode = (typeof window !== 'undefined')
    ...
    return (
        <div className={styles.root}>
            {askForUsername === true ? (
                <div className={`${styles.lobby} auroraBg`}>
                    <div className={styles.lobbyInner}>
                        ...
                        <div className={styles.lobbyContent}>
                            <div className={styles.lobbyPreview}>
                                ...
                                {!videoAvailable && (
                                    <div className={styles.lobbyCamOff}>
                                        <VideocamOffIcon sx={{ fontSize: 48, opacity: 0.6 }} />
                                        <span>Camera is off</span>
                                    </div>
                                )}
                                <div className={styles.lobbyPreviewControls}>
                                    <IconButton onClick={toggleLobbyAudio} className={styles.previewBtn} sx={{ bgcolor: audioAvailable ? 'rgba(255,255,255,0.12)' : '#ef4444' }}>
                                        {audioAvailable ? <MicIcon /> : <MicOffIcon />}
                                    </IconButton>
                                    <IconButton onClick={toggleLobbyVideo} className={styles.previewBtn} sx={{ bgcolor: videoAvailable ? 'rgba(255,255,255,0.12)' : '#ef4444' }}>
                            ...
                            <div className={styles.lobbyForm}>
                                <h1>Ready to join?</h1>
                                <p className={styles.lobbyCode}>Room <code>{meetingCode}</code></p>
                                <p className={styles.lobbySub}>Set your name and check your camera and mic before you join.</p>
                                <TextField
                                    label="Your name"
                                    value={username}
                ...
                <div className={styles.meetVideoContainer}>
                    ...
                    <div className={styles.topBar}>
                        <Logo size={28} onClick={() => (window.location.href = '/home')} />
                    ...
                    {showModal && (
                        <div className={styles.chatRoom}>
                            <div className={styles.chatHeader}>
                                <h2>In-call chat</h2>
                                <IconButton size="small" onClick={() => setModal(false)} sx={{ color: 'var(--text)' }}>
                                    <CloseIcon />
                                </IconButton>
                            </div>
                            ...
                            <div className={styles.chattingArea}>
                                <TextField
                                    ...
                                    fullWidth
                                />
                                <IconButton onClick={sendMessage} sx={{ color: 'var(--brand-400)' }}>
                                    <SendIcon />
                                </IconButton>
                            </div>
                        </div>
                    ...
                    <div className={styles.conferenceView}>
                        <div className={styles.videoTile}>
                            <video className={styles.tileVideo} ref={localVideoref} autoPlay muted></video>
                            <span className={styles.tileName}>{username || 'You'} (You)</span>
                        </div>
                        {videos.map((video) => (
                            <div className={styles.videoTile} key={video.socketId}>
                                <video
                                    ...
                                    autoPlay
                                ></video>
                                <span className={styles.tileName}>Participant</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className={styles.buttonContainers}>
                        ...
                        <Tooltip title="Chat">
                            <IconButton onClick={toggleChat} className={styles.ctrlBtn}>
                        ...
                        <Tooltip title="Leave call">
                            <IconButton onClick={handleEndCall} className={`${styles.ctrlBtn} ${styles.endBtn}`}>
                                <CallEndIcon />
                            </IconButton>
                        </Tooltip>
                    </div>
